...
const IPFS_GATEWAY = import.meta.env.VITE_IPFS_GATEWAY || "https://gateway.filebase.io/ipfs";
...
export async function uploadToIPFS(formData: FormData, fileType: 'image' | 'metadata'): Promise<IPFSResult> {
  ...
  try {
    ...
    throw new Error(`Failed to upload ${fileType} to IPFS: ${error.response?.data?.Message || error.message}`);
  }
}

export async function cleanupIPFSFiles(imageCid?: string, metadataCid?: string): Promise<void> {
  const cleanup = async (cid: string) => {
    try {
      await axios.post(`${IPFS_DELETE_URL}?arg=${cid}`, {}, {
        headers: {
          'Authorization': `Bearer ${FILEBASE_API_KEY}`,
        },
      });
      ...
      console.error(`❌ Failed to cleanup IPFS file ${cid}:`, error.response?.data || error.message);
